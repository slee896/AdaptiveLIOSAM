...
typedef pcl::PointXYZI PointType; // Members: float x, y, z, intensity
...
public:
    ...
    ParamServer()
    {
        ...
        nh.param<int>("lio_sam/downsampleRate", downsampleRate, 1); // default 1;
...
template<typename T>
sensor_msgs::PointCloud2 publishCloud3(const ros::Publisher& thisPub, const T& thisCloud, ros::Time thisStamp, std::string thisFrame)
{
    sensor_msgs::PointCloud2 tempCloud;
    pcl::toROSMsg(*thisCloud, tempCloud);
    tempCloud.header.stamp = thisStamp;
...
void pointsRawCallback(const sensor_msgs::PointCloud2::ConstPtr& msg);
